...
#              -PullLog git_pull.log -CommitLog git_pull_commit.log -ProjectId 1
#
# Parameters:
#   PullLog   — path to the file containing raw git pull output (default: git_pull.log)
#   CommitLog — optional path to a file with commit details line (e.g. from git log --oneline)
#               appended to raw_text so the parser can extract hash/author/message
...
if ($CommitLog -and (Test-Path $CommitLog)) {
    $commitContent = Get-Content $CommitLog -Raw -ErrorAction SilentlyContinue
    if ($commitContent) {
        $rawText += $commitContent
    }
}
